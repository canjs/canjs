...
define('can/component/component_test', [
    ...
    test('treecombo', function () {
        ...
        setTimeout(function () {
            ...
            var treecombo = can.$('#qunit-test-area treecombo')[0], breadcrumb = can.$('#qunit-test-area .breadcrumb')[0], breadcrumbLIs = breadcrumb.getElementsByTagName('li'), options = can.$('#qunit-test-area .options')[0], optionsLis = options.getElementsByTagName('li');
    ...
    asyncTest('stache integration', function () {
        ...
        var mustacheFrag = mustache(data), mustacheResult = mustacheFrag.childNodes[0].innerHTML.split(',');
    ...
    test('can.Construct are passed normally', function () {
        ...
        can.Component.extend({ tag: 'con-struct' });
...
/*can/list/promise/promise_test*/
define('can/list/promise/promise_test', [
...
/*can/observe/observe_test*/
define('can/observe/observe_test', [
    'can/util/util',
    'can/observe/observe',
    'can/map/map',
    'can/list/list',
...
define('can/compute/compute_test', [
    ...
    test('inner computes values are not bound to', function () {
        var num = can.compute(1), numBind = num.bind, numUnbind = num.unbind;
        var bindCount = 0;
        num.bind = function () {
            bindCount++;
...
define('can/model/model_test', [
    ...
    test('Creating nested models adds them to the store (#357)', function () {
        ...
        Person = can.Model.extend({ attributes: { company: 'Company.model' } }, {});
...
define('can/view/view_test', [
    ...
    test('basic loading', function () {
        var data = { message: 'hello' }, expected = '<h1>hello</h1>', templates = {
                'ejs': '<h1><%= message %></h1>',
    ...
    test('nameless renderers (#162, #195)', 8, function () {
        var nameless = can.view.ejs('<h2><%= message %></h2>'), data = { message: 'HI!' }, result = nameless(data), node = result.childNodes[0];
    ...
    test('Select live bound options don\'t contain __!!__', function () {
        var domainList = new can.List([
            ...
            ]), frag = can.view(can.test.path('view/test/select.ejs'), { domainList: domainList }), div = document.createElement('div');
        div.appendChild(frag);
    ...
    test('live binding textNodes before a table', function () {
        var data = new can.Map({ loading: true }), templates = {
    ...
    test('Resetting a live-bound <textarea> changes its value to __!!__ (#223)', function () {
        var template = can.view.ejs('<form><textarea><%= this.attr(\'test\') %></textarea></form>'), frag = template(new can.Map({ test: 'testing' })), form, textarea;
        can.append(can.$('#qunit-test-area'), frag);
    ...
    test('Deferred fails (#276)', function () {
        var foo = new can.Deferred();
        stop();
        can.view.render(can.test.path('view/test/deferred.ejs'), foo).fail(function (error) {
    ...
    test('Using \'=\' in attribute does not truncate the value', function () {
        var template = can.view.ejs('<img id=\'equalTest\' <%= this.attr(\'class\') %> src="<%= this.attr(\'src\') %>">'), obs = new can.Map({
...
/*can/view/ejs/ejs_test*/
define('can/view/ejs/ejs_test', [
    'can/model/model',
    'can/view/ejs/ejs',
    'can/test/test'
], function () {
    ...
    test('multi line', function () {
        var text = 'a \n b \n c', result = new can.EJS({ text: text }).render({});
        equal(result, text);
    });
    test('multi line elements', function () {
        var text = '<img\n class="<%=myClass%>" />', result = new can.EJS({ text: text }).render({ myClass: 'a' });
    ...
    test('multiple function hookups in a tag', function () {
        var text = '<span <%= (el)-> can.data(can.$(el),\'foo\',\'bar\') %>' + ' <%= (el)-> can.data(can.$(el),\'baz\',\'qux\') %>>lorem ipsum</span>', compiled = new can.EJS({ text: text }).render(), div = document.createElement('div');
    ...
    test('list helper', function () {
        ...
        var todos = new can.List([{
                    id: 1,
                    name: 'Dishes'
                }]), compiled = new can.EJS({ text: text }).render({ todos: todos }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('select live binding', function () {
        var text = '<select><% todos.each(function(todo){ %><option><%= todo.name %></option><% }) %></select>', Todos = new can.List([{
                    id: 1,
                    name: 'Dishes'
                }]), compiled = new can.EJS({ text: text }).render({ todos: Todos }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('multiple tbodies in table hookup', function () {
        var text = '<table>' + '<% can.each(people, function(person){ %>' + '<tbody><tr><td><%= person.name %></td></tr></tbody>' + '<% }) %>' + '</table>', people = new can.List([
                { name: 'Steve' },
                { name: 'Doug' }
            ]), compiled = new can.EJS({ text: text }).render({ people: people }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        equal(div.getElementsByTagName('tbody').length, 2, 'two tbodies');
    });
    test('multiple hookups in a single attribute', function () {
        var text = '<div class=\'<%= obs.attr("foo") %>a<%= obs.attr("bar") %>b<%= obs.attr("baz") %>\'></div>', obs = new can.Map({
                foo: '1',
                bar: '2',
                baz: '3'
    ...
    test('adding and removing multiple html content within a single element', function () {
        var text = '<div><%== obs.attr("a") %><%== obs.attr("b") %><%== obs.attr("c") %></div>', obs = new can.Map({
                a: 'a',
                b: 'b',
                c: 'c'
            }), compiled = new can.EJS({ text: text }).render({ obs: obs });
    ...
    test('live binding and removeAttr', function () {
        var text = '<% if(obs.attr("show")) { %>' + '<p <%== obs.attr("attributes") %> class="<%= obs.attr("className")%>"><span><%= obs.attr("message") %></span></p>' + '<% } %>', obs = new can.Map({
                show: true,
                className: 'myMessage',
                attributes: 'some="myText"',
                message: 'Live long and prosper'
            }), compiled = new can.EJS({ text: text }).render({ obs: obs }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('hookup within a tag', function () {
        var text = '<div <%== obs.attr("foo") %> ' + '<%== obs.attr("baz") %>>lorem ipsum</div>', obs = new can.Map({
                foo: 'class="a"',
                baz: 'some=\'property\''
            }), compiled = new can.EJS({ text: text }).render({ obs: obs });
        var div = document.createElement('div');
    ...
    test('single escaped tag, removeAttr', function () {
        var text = '<div <%= obs.attr("foo") %>>lorem ipsum</div>', obs = new can.Map({ foo: 'data-bar="john doe\'s bar"' }), compiled = new can.EJS({ text: text }).render({ obs: obs });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var anchor = div.getElementsByTagName('div')[0];
    ...
    test('html comments', function () {
        var text = '<!-- bind to changes in the todo list --> <div><%= obs.attr("foo") %></div>', obs = new can.Map({ foo: 'foo' }), compiled = new can.EJS({ text: text }).render({ obs: obs });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        equal(div.getElementsByTagName('div')[0].innerHTML, 'foo', 'Element as expected');
    });
    test('hookup and live binding', function () {
        var text = '<div class=\'<%= task.attr(\'completed\') ? \'complete\' : \'\' %>\' <%= (el)-> can.data(can.$(el),\'task\',task) %>>' + '<%== task.attr(\'name\') %>' + '</div>', task = new can.Map({
                completed: false,
                className: 'someTask',
                name: 'My Name'
            }), compiled = new can.EJS({ text: text }).render({ task: task }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('unescape bindings change', function () {
        ...
        var text = '<div><%== completed() %></div>', compiled = new can.EJS({ text: text }).render({ completed: completed });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var child = div.getElementsByTagName('div')[0];
    ...
    test('attribute value bindings change', function () {
        ...
        var text = '<div items="<%= completed() %>"></div>', compiled = new can.EJS({ text: text }).render({ completed: completed });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var child = div.getElementsByTagName('div')[0];
        equal(child.getAttribute('items'), '2', 'at first there are 2 true bindings');
    ...
    test('parent is right with bock', function () {
        var text = '<ul><% if(!obs.attr("items").length) { %>' + '<li>No items</li>' + '<% } else { %> <%== obs.attr("content") %>' + '<% } %></ul>', obs = new can.Map({
                content: '<li>Hello</li>',
                items: [{ name: 'Justin' }]
            }), compiled = new can.EJS({ text: text }).render({ obs: obs });
        var div = document.createElement('div');
    ...
    test('recursive views of previously stolen files shouldn\'t fail', function () {
        can.view.preloadStringRenderer('view_ejs_test_indirect1_ejs', can.EJS({ text: '<ul>' + '<% unordered.each(function(item) { %>' + '<li>' + '<% if(item.ol) { %>' + '<%== can.view.render(can.test.path(\'view/ejs/test/indirect2.ejs\'), { ordered: item.ol }) %>' + '<% } else { %>' + '<%= item.toString() %>' + '<% } %>' + '</li>' + '<% }) %>' + '</ul>' }));
        can.view.preloadStringRenderer('view_ejs_test_indirect2_ejs', can.EJS({ text: '<ol>' + '<% ordered.each(function(item) { %>' + '<li>' + '<% if(item.ul) { %>' + '<%== can.view.render(can.test.path(\'view/ejs/test/indirect1.ejs\'), { unordered: item.ul }) %>' + '<% } else { %>' + '<%= item.toString() %>' + '<% } %>' + '</li>' + '<% }) %>' + '</ol>' }));
    ...
    test('live binding select', function () {
        var text = '<select><% items.each(function(ob) { %>' + '<option value=\'<%= ob.attr(\'id\') %>\'><%= ob.attr(\'title\') %></option>' + '<% }); %></select>', items = new can.List([
                {
                    title: 'Make bugs',
                    is_done: true,
                    id: 0
            ...
            ]), compiled = new can.EJS({ text: text }).render({ items: items }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('live binding textarea', function () {
        ...
        var obs = new can.Map({ middle: 'yes' }), div = document.createElement('div');
    ...
    test('reset on a live bound input', function () {
        var text = '<input type=\'text\' value=\'<%= person.attr(\'name\') %>\'><button type=\'reset\'>Reset</button>', person = new can.Map({ name: 'Bob' }), compiled = new can.EJS({ text: text }).render({ person: person }), form = document.createElement('form'), input;
    ...
    test('A non-escaping live magic tag within a control structure and no leaks', function () {
        ...
        var text = '<div><% items.each(function(ob) { %>' + '<%== ob.attr(\'html\') %>' + '<% }); %></div>', items = new can.List([{ html: '<label>Hello World</label>' }]), compiled = new can.EJS({ text: text }).render({ items: items }), div = can.$('#qunit-test-area')[0];
        div.innerHTML = '';
        can.append(can.$('#qunit-test-area'), can.view.frag(compiled));
    ...
    test('attribute unquoting', function () {
        var text = '<input type="radio" ' + '<%== facet.single ? \'name="facet-\' + facet.attr("id") + \'"\' : "" %> ' + 'value="<%= facet.single ? "facet-" + facet.attr("id") : "" %>" />', facet = new can.Map({
                id: 1,
                single: true
            }), compiled = new can.EJS({ text: text }).render({ facet: facet }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('empty element hooks work correctly', function () {
        ...
        var compiled = new can.EJS({ text: text }).render(), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('spaces between attribute name and value', function () {
        var text = '<input type="text" value = "<%= test %>" />', compiled = new can.EJS({ text: text }).render({ test: 'testing' }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var input = div.getElementsByTagName('input')[0];
    ...
    test('live binding with computes', function () {
        var text = '<span><%= compute() %></span>', compute = can.compute(5), compiled = new can.EJS({ text: text }).render({ compute: compute }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('testing for clean tables', function () {
        ...
        var res = can.view.render(can.test.path('view/ejs/test/table_test.ejs'), { games: games }), div = document.createElement('div');
    ...
    test('inserting live-binding partials assume the correct parent tag', function () {
        ...
        equal(can.view.render('tableView', data).indexOf('<table><tbody><tr><td data-view-id='), 0, 'Rendered output starts' + 'as expected');
        can.view.hookups = {};
    });
    ...
    test('live binding with html comment', function () {
        var text = '<table><tr><th>Todo</th></tr><!-- do not bother with me -->' + '<% todos.each(function(todo){ %><tr><td><%= todo.name %></td></tr><% }) %></table>', Todos = new can.List([{
                    id: 1,
                    name: 'Dishes'
                }]), compiled = new can.EJS({ text: text }).render({ todos: Todos }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
    ...
    test('HTML comment with element callback', function () {
        var text = [
                ...
                '</ul>'
                    ...
                    id: 1,
                    name: 'Dishes'
                }]), compiled = new can.EJS({ text: text.join('\n') }).render({ todos: Todos }), div = document.createElement('div'), li, comments;
    ...
    test('Interpolated values when iterating through an Observe.List should still render when not surrounded by a DOM node', function () {
        ...
        var todos = [
            ...
            ], data = { todos: new can.List(todos) }, arr = { todos: todos }, div = document.createElement('div');
    ...
    test('correctness of data-view-id and only in tag opening', function () {
        var text = [
            ...
            ], compiled = new can.EJS({ text: text.join('') }).render({ items: items }), expected = '^<textarea data-view-id=\'[0-9]+\'><select><option data-view-id=\'[0-9]+\'>One</option>' + '<option data-view-id=\'[0-9]+\'>Two</option></select></textarea>$';
    ...
    test('list works within another branch', function () {
        var animals = new can.List([]), template = '<div>Animals:' + '<% if( animals.attr(\'length\') ){ %>~' + '<% animals.each(function(animal){%>' + '<span><%=animal %></span>' + '<%})%>' + '<% } else { %>' + 'No animals' + '<% } %>' + '!</div>';
        var renderer = can.view.ejs(template);
        var div = document.createElement('div');
        var frag = renderer({ animals: animals });
        div.appendChild(frag);
    ...
    test('each works within another branch', function () {
        var animals = new can.List([]), template = '<div>Animals:' + '<% if( animals.attr(\'length\') ){ %>~' + '<%==each(animals, function(animal){%>' + '<span><%=animal %></span>' + '<%})%>' + '<% } else { %>' + 'No animals' + '<% } %>' + '!</div>';
        var renderer = can.view.ejs(template);
        var div = document.createElement('div');
        var frag = renderer({ animals: animals });
        div.appendChild(frag);
    ...
    test('JS blocks within EJS tags shouldn\'t require isolation', function () {
        var isolatedBlocks = can.view.ejs('<% if (true) { %>' + '<% if (true) {%>' + 'hi' + '<% } %>' + '<% } %>'), sharedBlocks = can.view.ejs('<% if (true) { %>' + '<% if (true) { %>' + 'hi' + '<% }' + '} %>'), complexIsolatedBlocks = can.view.ejs('<% if (true) { %><% if (1) { %>' + '<% if ({ dumb: \'literal\' }) { %>' + '<% list(items, function(item) { %>' + '<%== item %>' + '<%== something(function(items){ %><%== items.length %><% }) %>' + '<% }) %>' + '<% } %>' + '<% } %><% } %>'), complexSharedBlocks = can.view.ejs('<% if (true) { if (1) { %>' + '<% if ({ dumb: \'literal\' }) { %>' + '<% list(items, function(item) { %>' + '<%== item %>' + '<%== something(function(items){ %><%== items.length %><% }) %>' + '<% }) %>' + '<% }' + '} } %>'), iteratedSharedBlocks = can.view.ejs('<% for (var i = 0; i < items.length; i++) { %>' + '<% if (this.items) { if (1) { %>' + 'hi' + '<% } } else { %>' + 'nope' + '<% } } %>'), iteratedString = can.view.ejs('<% for(var i = 0; i < items.length; i++) { %>' + '\t<% if(this.mode !== "RESULTS") {' + '\t\tif(items[i] !== "SOME_FAKE_VALUE") { %>' + '\t\t\thi' + '\t\t<% }' + '\t} else { %>' + '\t\tnope' + '\t<% }' + '} %>'), iteratedStringNewLines = can.view.ejs('<% for(var i = 0; i < items.length; i++) { %>' + '\t<% if(this.mode !== "RESULTS") {\n' + '\t\tif(items[i] !== "SOME_FAKE_VALUE") { %>' + '\t\t\thi' + '\t\t<% }\n' + '\t} else { %>' + '\t\tnope' + '\t<% }\n' + '} %>'), data = {
                items: [
                    'one',
                    'two',
                    'three'
    ...
    test('Access .length with nested dot notation', function () {
        var template = '<span id="nested"><%= this.attr("list.length") %></span>' + '<span id="unnested"><%= this.list.attr("length") %></span>', obj = new can.Map({
                list: [
                    0,
                    1,
                    2,
            ...
            }), renderer = can.view.ejs(template), div = document.createElement('div');
    ...
    test('_bindings removed when element removed', function () {
        var template = can.view.ejs('<div id="game"><% if(game.attr("league")) { %><%= game.attr("name") %><% } %></div>'), game = new can.Map({
...
define('can/view/stache/stache_test', [
    ...
    test('hookup and live binding', function () {
        var text = '<div class=\'{{ task.completed }}\' {{ data \'task\' task }}>' + '{{ task.name }}' + '</div>', task = new can.Map({
                ...
                className: 'someTask',
                name: 'My Name'
            }), compiled = can.stache(text)({ task: task }), div = document.createElement('div');
        div.appendChild(compiled);
        var child = div.getElementsByTagName('div')[0];
    ...
    test('Functions and helpers should be passed the same context', function () {
        ...
        var renderer = can.stache('"{{next_level.text}}" uppercased should be "<span>{{to_upper next_level.text}}</span>"<br/>"{{next_level.text}}" uppercased with a workaround is "<span>{{#to_upper}}{{next_level.text}}{{/to_upper}}</span>"'), data = {
                next_level: {
                    text: function () {
    ...
    test('Interpolated values when iterating through an Observe.List should still render when not surrounded by a DOM node', function () {
        var renderer = can.stache('{{ #todos }}{{ name }}{{ /todos }}'), renderer2 = can.stache('{{ #todos }}<span>{{ name }}</span>{{ /todos }}'), todos = [
            ...
            ], liveData = { todos: new can.List(todos) }, plainData = { todos: todos }, div = document.createElement('div');
    ...
    test('objects with a \'key\' or \'index\' property should work in helpers', function () {
        var renderer = can.stache('{{ #obj }}{{ show_name }}{{ /obj }}'), div = document.createElement('div');
    ...
    test('2 way binding helpers', function () {
        ...
        var div = document.createElement('div'), u = new can.Map({ name: 'Justin' });
    ...
    test('HTML comment with helper', function () {
        var text = [
                ...
                '</ul>'
                    ...
                    id: 1,
                    name: 'Dishes'
                }]), compiled = can.stache(text.join('\n'))({ todos: todos }), div = document.createElement('div'), li;
    ...
    test('Empty strings in arrays within Observes that are iterated should return blank strings', function () {
        var data = new can.Map({
            ...
            }), compiled = can.stache('<select>{{#colors}}<option>{{.}}</option>{{/colors}}</select>')(data), div = document.createElement('div');
        div.appendChild(compiled);
    ...
    test('Null properties do not throw errors', function () {
        var renderer = can.stache('Foo bar {{#foo.bar}}exists{{/foo.bar}}{{^foo.bar}}does not exist{{/foo.bar}}'), div = document.createElement('div'), div2 = document.createElement('div'), frag, frag2;
        try {
    ...
    test('Data helper should set proper data instead of a context stack', function () {
        ...
        var renderer = can.stache('{{#bar}}{{> #nested_data}}{{/bar}}'), renderer2 = can.stache('{{#bar}}{{> #nested_data2}}{{/bar}}'), renderer3 = can.stache('{{#bar}}{{> #nested_data3}}{{/bar}}'), div = document.createElement('div'), data = new can.Map({
                foo: 'bar',
                bar: new can.Map({})
            }), span;
        div.innerHTML = '';
    ...
    test('Functions passed to default helpers should be evaluated', function () {
        var renderer = can.stache('{{#if hasDucks}}Ducks: {{ducks}}{{else}}No ducks!{{/if}}'), div = document.createElement('div'), data = new can.Map({
                ducks: '',
                hasDucks: function () {
    ...
    test('hiding image srcs (#157)', function () {
        ...
        var frag = template(data), img = frag.childNodes[0];
    ...
    test('directly nested subitems and each (#605)', function () {
        ...
        var frag = template(data), div = frag.childNodes[0], labels = div.getElementsByTagName('label');
    ...
    test('{{#each}} helper works reliably with nested sections (#604)', function () {
        ...
        var div = document.createElement('div'), lis = div.getElementsByTagName('li');
        div.appendChild(renderer(data));
    ...
    test('Block bodies are properly escaped inside attributes', function () {
        var html = '<div title=\'{{#test}}{{.}}{{{.}}}{{/test}}\'></div>', div = document.createElement('div'), title = 'Alpha&Beta';
        div.appendChild(can.stache(html)(new can.Map({ test: title })));
    ...
    test('{{#each}} handles an undefined list changing to a defined list (#629)', function () {
        ...
        var div = document.createElement('div'), data1 = new can.Map({ description: 'Each without list' }), data2 = new can.Map({
    ...
    test('can.compute should live bind when the value is changed to a Construct (#638)', function () {
        var renderer = can.stache('<p>{{#counter}} Clicked <span>{{count}}</span> times {{/counter}}</p>'), div = document.createElement('div'), counter = can.compute(), data = { counter: counter };
        div.appendChild(renderer(data));
    ...
    test('@index in partials loaded from script templates', function () {
        ...
        var frag = itemsTemplate({ items: items }), div = frag.childNodes[0], labels = div.getElementsByTagName('label');
...
define('can/control/control_test', ['can/control/control'], function () {
...
/*can/route/route_test*/
define('can/route/route_test', [
    'can/route/route',
    'can/test/test'
], function () {
...
/*can/view/mustache/mustache_test*/
define('can/view/mustache/mustache_test', [
    'can/model/model',
    'can/view/mustache/mustache',
    'can/test/test',
    'can/view/mustache/spec/specs/specs'
    ...
    QUnit.module('can/view/mustache, rendering', {
        setup: function () {
            ...
            this.squareBrackets = '<ul>{{#animals}}' + '<li>{{.}}</li>' + '{{/animals}}</ul>';
            this.squareBracketsNoThis = '<ul>{{#animals}}' + '<li>{{.}}</li>' + '{{/animals}}</ul>';
            this.angleBracketsNoThis = '<ul>{{#animals}}' + '<li>{{.}}</li>' + '{{/animals}}</ul>';
        }
    });
    var override = {
    ...
    test('Mustache live-binding with escaping', function () {
        var template = '<span id=\'binder1\'>{{ name }}</span><span id=\'binder2\'>{{{name}}}</span>';
        var teacher = new can.Map({ name: '<strong>Mrs Peters</strong>' });
        var tpl = new can.Mustache({ text: template });
    ...
    test('No arguments passed to helper with list', function () {
        ...
        var div = document.createElement('div');
        div.appendChild(can.view('noargs', { items: new can.List([{ name: 'Brian' }]) }, {
            noargHelper: function () {
                return 'foo';
            }
        }));
    ...
    test('Partials and observes', function () {
        var template;
        var div = document.createElement('div');
        template = can.view.mustache('table', '<table><thead><tr>{{#data}}{{{>' + can.test.path('view/mustache/test/partial.mustache') + '}}}{{/data}}</tr></thead></table>');
    ...
    test('multi line', function () {
        var text = 'a \n b \n c', result = new can.Mustache({ text: text }).render({});
        equal(result, text);
    });
    test('multi line elements', function () {
        var text = '<img\n class="{{myClass}}" />', result = new can.Mustache({ text: text }).render({ myClass: 'a' });
        ok(result.indexOf('<img\n class="a"') !== -1, 'Multi-line elements render correctly.');
        can.view.hookups = {};
    });
    test('escapedContent', function () {
    ...
    test('multiple function hookups in a tag', function () {
        var text = '<span {{(el)-> can.data(can.$(el),\'foo\',\'bar\')}}' + ' {{(el)-> can.data(can.$(el),\'baz\',\'qux\')}}>lorem ipsum</span>', compiled = new can.Mustache({ text: text }).render(), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var span = div.getElementsByTagName('span')[0];
        equal(can.data(can.$(span), 'foo'), 'bar', 'first hookup');
        equal(can.data(can.$(span), 'baz'), 'qux', 'second hookup');
    ...
    test('live binding and removeAttr', function () {
        var text = '{{ #obs.show }}' + '<p {{ obs.attributes }} class="{{ obs.className }}"><span>{{ obs.message }}</span></p>' + '{{ /obs.show }}', obs = new can.Map({
                ...
                attributes: 'some="myText"',
                message: 'Live long and prosper'
            }), compiled = new can.Mustache({ text: text }).render({ obs: obs }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var p = div.getElementsByTagName('p')[0], span = p.getElementsByTagName('span')[0];
        equal(p.getAttribute('some'), 'myText', 'initial render attr');
        equal(getAttr(p, 'class'), 'myMessage', 'initial render class');
    ...
    test('single escaped tag, removeAttr', function () {
        var text = '<div {{ obs.foo }}>lorem ipsum</div>', obs = new can.Map({ foo: 'data-bar="john doe\'s bar"' }), compiled = new can.Mustache({ text: text }).render({ obs: obs });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var anchor = div.getElementsByTagName('div')[0];
        equal(anchor.getAttribute('data-bar'), 'john doe\'s bar');
    ...
    test('hookup and live binding', function () {
        var text = '<div class=\'{{ task.completed }}\' {{ (el)-> can.data(can.$(el),\'task\',task) }}>' + '{{ task.name }}' + '</div>', task = new can.Map({
                completed: false,
                className: 'someTask',
                name: 'My Name'
            }), compiled = new can.Mustache({ text: text }).render({ task: task }), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var child = div.getElementsByTagName('div')[0];
        ok(child.className.indexOf('false') > -1, 'is incomplete');
        ok(!!can.data(can.$(child), 'task'), 'has data');
    ...
    test('multiple curly braces in a block', function () {
        var text = '{{^obs.items}}' + '<li>No items</li>' + '{{/obs.items}}' + '{{#obs.items}}' + '<li>{{name}}</li>' + '{{/obs.items}}', obs = new can.Map({ items: [] }), compiled = new can.Mustache({ text: text }).render({ obs: obs });
        var ul = document.createElement('ul');
    ...
    test('unescape bindings change', function () {
        ...
        var text = '<div>{{ completed }}</div>', compiled = new can.Mustache({ text: text }).render({ completed: completed });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var child = div.getElementsByTagName('div')[0];
        equal(child.innerHTML, '2', 'at first there are 2 true bindings');
    ...
    test('attribute value bindings change', function () {
        ...
        var text = '<div items="{{{ completed }}}"></div>', compiled = new can.Mustache({ text: text }).render({ completed: completed });
        var div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        var child = div.getElementsByTagName('div')[0];
        equal(child.getAttribute('items'), '2', 'at first there are 2 true bindings');
    ...
    test('computes are supported in default helpers', function () {
        ...
        for (result in staches) {
            ...
            div = document.createElement('div');
            actual = can.view('compute_' + result, { test: can.compute(data) });
            div.appendChild(actual);
            span = div.getElementsByTagName('span')[0];
            if (span && span.firstChild) {
                div.replaceChild(span.firstChild, span);
    ...
    test('Rendering models in tables produces different results than an equivalent observe (#202)', 2, function () {
        ...
        var div = document.createElement('div');
        var dom = renderer({ stuff: new can.Map({ rows: [{ name: 'first' }] }) });
        div.appendChild(dom);
        var elements = div.getElementsByTagName('tbody');
        equal(elements.length, 1, 'Only one <tbody> rendered');
        div = document.createElement('div');
        dom = renderer({ stuff: new can.Model({ rows: [{ name: 'first' }] }) });
        div.appendChild(dom);
    ...
    test('Contexts within helpers not always resolved correctly', function () {
        ...
        var renderer = can.view.mustache('{{#bad_context next_level}}<span>{{text}}</span><br/><span>{{other_text}}</span>{{/bad_context}}'), data = {
                next_level: {
                    text: 'bar',
                    other_text: 'In the inner context'
                },
    ...
    test('Contexts are not always passed to partials properly', function () {
        ...
        var renderer = can.view.mustache('{{#first_level}}<span>{{> inner}}</span> should equal <span>{{other_first_level}}</span>{{/first_level}}'), data = {
                first_level: { second_level: 'bar' },
                other_first_level: 'foo'
            }, div = document.createElement('div');
        div.appendChild(renderer(data));
    ...
    test('Functions and helpers should be passed the same context', function () {
        ...
        var renderer = can.view.mustache('"{{next_level.text}}" uppercased should be "<span>{{to_upper next_level.text}}</span>"<br/>"{{next_level.text}}" uppercased with a workaround is "<span>{{#to_upper}}{{next_level.text}}{{/to_upper}}</span>"'), data = {
                next_level: {
                    text: function () {
                        return this.other_text;
                    },
    ...
    test('Interpolated values when iterating through an Observe.List should still render when not surrounded by a DOM node', function () {
        var renderer = can.view.mustache('{{ #todos }}{{ name }}{{ /todos }}'), renderer2 = can.view.mustache('{{ #todos }}<span>{{ name }}</span>{{ /todos }}'), todos = [
                {
                    id: 1,
                    name: 'Dishes'
                },
    ...
    test('objects with a \'key\' or \'index\' property should work in helpers', function () {
        var renderer = can.view.mustache('{{ #obj }}{{ show_name }}{{ /obj }}'), div = document.createElement('div');
        div.appendChild(renderer({
            obj: {
                id: 2,
                name: 'Forks',
    ...
    test('can pass in partials', function () {
        ...
        var result = hello.render({ name: 'World' }, { partials: { name: fancyName } });
    ...
    test('can pass in helpers', function () {
        ...
        var result = helpers.render({ name: 'world' }, {
                helpers: {
                    cap: function (name) {
                        return can.capitalize(name);
                    }
    ...
    test('HTML comment with helper', function () {
        var text = [
                ...
                '</ul>'
                    ...
                    id: 1,
                    name: 'Dishes'
                }]), compiled = new can.Mustache({ text: text.join('\n') }).render({ todos: Todos }), div = document.createElement('div'), li;
        var comments = function (el) {
            var count = 0;
            for (var i = 0; i < el.childNodes.length; i++) {
                if (el.childNodes[i].nodeType === 8) {
    ...
    test('correctness of data-view-id and only in tag opening', function () {
        var text = [
            ...
            ], compiled = new can.Mustache({ text: text.join('') }).render({ items: items }), expected = '^<textarea data-view-id=\'[0-9]+\'><select><option data-view-id=\'[0-9]+\'>One</option>' + '<option data-view-id=\'[0-9]+\'>Two</option></select></textarea>$';
        ok(compiled.search(expected) === 0, 'Rendered output is as expected');
        can.view.hookups = {};
    });
    test('Empty strings in arrays within Observes that are iterated should return blank strings', function () {
        var data = new can.Map({
            ...
            }), compiled = new can.Mustache({ text: '<select>{{#colors}}<option>{{.}}</option>{{/colors}}</select>' }).render(data), div = document.createElement('div');
        div.appendChild(can.view.frag(compiled));
        equal(div.getElementsByTagName('option')[0].innerHTML, '', 'Blank string should return blank');
    });
    test('Null properties do not throw errors in Mustache.get', function () {
        var renderer = can.view.mustache('Foo bar {{#foo.bar}}exists{{/foo.bar}}{{^foo.bar}}does not exist{{/foo.bar}}'), div = document.createElement('div'), div2 = document.createElement('div'), frag, frag2;
        try {
            frag = renderer(new can.Map({ foo: null }));
        } catch (e) {
            ok(false, 'rendering with null threw an error');
    ...
    test('Data helper should set proper data instead of a context stack', function () {
        ...
        var renderer = can.view.mustache('{{#bar}}{{> #nested_data}}{{/bar}}'), renderer2 = can.view.mustache('{{#bar}}{{> #nested_data2}}{{/bar}}'), renderer3 = can.view.mustache('{{#bar}}{{> #nested_data3}}{{/bar}}'), div = document.createElement('div'), data = new can.Map({
                foo: 'bar',
                bar: new can.Map({})
            }), span;
        div.innerHTML = '';
    ...
    test('Functions passed to default helpers should be evaluated', function () {
        var renderer = can.view.mustache('{{#if hasDucks}}Ducks: {{ducks}}{{else}}No ducks!{{/if}}'), div = document.createElement('div'), data = new can.Map({
                ducks: '',
                hasDucks: function () {
                    return this.attr('ducks').length > 0;
                }
    ...
    test('Each works with the empty list', function () {
        var animals = new can.List([]), renderer = can.view.mustache('<div>my<b>favorite</b>animals:{{#each animals}}<label>Animal=</label> <span>{{this}}</span>{{/}}!</div>');
        var div = document.createElement('div');
        var frag = renderer({ animals: animals });
        div.appendChild(frag);
        animals.push('sloth', 'bear');
    ...
    test('each works within another branch', function () {
        var animals = new can.List([]), template = '<div>Animals:' + '{{#if animals.length}}~' + '{{#each animals}}' + '<span>{{.}}</span>' + '{{/each}}' + '{{else}}' + 'No animals' + '{{/if}}' + '!</div>';
    ...
    test('Object references can escape periods for key names containing periods', function () {
        var template = can.view.mustache('{{#foo.bar}}' + '{{some\\\\.key\\\\.name}} {{some\\\\.other\\\\.key.with\\\\.more}}' + '{{/foo.bar}}'), data = {
                foo: {
                    bar: [{
                            'some.key.name': 100,
                            'some.other.key': { 'with.more': 'values' }
    ...
    test('Computes should be resolved prior to accessing attributes', function () {
        var template = can.view.mustache('{{list.length}}'), data = { list: can.compute(new can.List()) };
        var div = document.createElement('div');
        div.appendChild(template(data));
        equal(div.innerHTML, '0');
    });
    test('Helpers can be passed . or this for the active context', function () {
        ...
        var template = can.view.mustache('{{#attendee}}{{#events}}<div>{{rsvp attendee .}}</div>{{/events}}{{/#attendee}}'), data = {
                attendee: { name: 'Justin' },
                events: [
                    { name: 'Reception' },
                    { name: 'Wedding' }
    ...
    test('hiding image srcs (#157)', function () {
        var template = can.view.mustache('<img {{#image}}src="{{.}}"{{/image}} alt="An image" />'), data = new can.Map({ image: null }), url = 'http://canjs.us/scripts/static/img/canjs_logo_yellow_small.png';
        var frag = template(data), img = frag.childNodes[0];
        equal(img.src, '', 'there is no src');
        data.attr('image', url);
        notEqual(img.src, '', 'Image should have src');
    ...
    test('live binding in a truthy section', function () {
        var template = can.view.mustache('<div {{#width}}width="{{.}}"{{/width}}></div>'), data = new can.Map({ width: '100' });
        var frag = template(data), img = frag.childNodes[0];
        equal(img.getAttribute('width'), '100', 'initial width is correct');
        data.attr('width', '300');
        equal(img.getAttribute('width'), '300', 'updated width is correct');
    ...
    test('backtracks in mustache (#163)', function () {
        var template = can.view.mustache('{{#grid.rows}}' + '{{#grid.cols}}' + '<div>{{columnData ../. .}}</div>' + '{{/grid.cols}}' + '{{/grid.rows}}');
        var grid = new can.Map({
                rows: [
                    {
                        first: 'Justin',
    ...
    test('passing can.List to helper (#438)', function () {
        var renderer = can.view.mustache('<ul><li {{helper438 observeList}}>observeList broken</li>' + '<li {{helper438 array}}>plain arrays work</li></ul>');
    ...
    test('hiding image srcs (#494)', function () {
        var template = can.view.mustache('<img src="{{image}}"/>'), data = new can.Map({ image: '' }), url = 'http://canjs.us/scripts/static/img/canjs_logo_yellow_small.png';
    ...
    test('hiding image srcs with complex content (#494)', function () {
        var template = can.view.mustache('<img src="{{#image}}http://{{domain}}/{{loc}}.png{{/image}}"/>'), data = new can.Map({}), imgData = {
                domain: 'canjs.us',
                loc: 'scripts/static/img/canjs_logo_yellow_small'
            }, url = 'http://canjs.us/scripts/static/img/canjs_logo_yellow_small.png';
        var str = template.render(data);
    ...
    test('can.mustache.safeString', function () {
        var text = 'Google', url = 'http://google.com/', templateEscape = can.view.mustache('{{link "' + text + '" "' + url + '"}}'), templateUnescape = can.view.mustache('{{{link "' + text + '" "' + url + '"}}}');
    ...
    test('each directly within live html section', function () {
        var tmp = can.view.mustache('<ul>{{#if showing}}' + '{{#each items}}<li>item</li>{{/items}}' + '{{/if}}</ul>');
        var items = new can.List([
                1,
                2,
                3
    ...
    test('can.Mustache.safestring works on live binding (#606)', function () {
        ...
        can.mustache.registerHelper('safeHelper', function () {
            return can.mustache.safeString('<p>' + num() + '</p>');
        });
    ...
    test('directly nested subitems and each (#605)', function () {
        var template = can.view.mustache('<div>' + '{{#item}}' + '<p>This is the item:</p>' + '{{#each subitems}}' + '<label>' + 'item' + '</label>' + '{{/each}}' + '{{/item}}' + '</div>');
        var data = new can.Map({ item: { subitems: ['first'] } });
        var frag = template(data), div = frag.childNodes[0], labels = div.getElementsByTagName('label');
        equal(labels.length, 1, 'initially one label');
        data.attr('item.subitems').push('second');
    ...
    test('directly nested live sections unbind without needing the element to be removed', function () {
        var template = can.view.mustache('<div>' + '{{#items}}' + '<p>first</p>' + '{{#visible}}<label>foo</label>{{/visible}}' + '<p>second</p>' + '{{/items}}' + '</div>');
        var data = new can.Map({ items: [{ visible: true }] });
        function handler(eventType) {
            can.Map.prototype.unbind.apply(this, arguments);
            if (eventType === 'visible') {
    ...
    test('Rendering keys of an object with #each and @key in a Component', function () {
        var template = can.view.mustache('<ul>' + '{{#each data}}' + '<li>{{@key}} : {{.}}</li>' + '{{/data}}' + '</ul>');
        var map = new can.Map({
                data: {
                    some: 'test',
                    things: false,
    ...
    test('{{#each}} helper works reliably with nested sections (#604)', function () {
        var renderer = can.view.mustache('{{#if first}}<ul>{{#each list}}<li>{{name}}</li>{{/each}}</ul>' + '{{else}}<ul>{{#each list2}}<li>{{name}}</li>{{/each}}</ul>{{/if}}');
        var data = new can.Map({
                first: true,
                list: [
                    { name: 'Something' },
    ...
    test('Block bodies are properly escaped inside attributes', function () {
        var html = '<div title=\'{{#test}}{{.}}{{{.}}}{{/test}}\'></div>', div = document.createElement('div'), title = 'Alpha&Beta';
        div.appendChild(can.view.mustache(html)(new can.Map({ test: title })));
        equal(div.getElementsByTagName('div')[0].title, title + title);
    });
    test('Constructor static properties are accessible (#634)', function () {
        can.Map.extend('can.Foo', { static_prop: 'baz' }, { proto_prop: 'thud' });
        var template = '\t\t\t\tStraight access: <br/> \t\t\t\t\t<span>{{own_prop}}</span><br/> \t\t\t\t\t<span>{{constructor.static_prop}}</span><br/> \t\t\t\t\t<span>{{constructor.proto_prop}}</span><br/> \t\t\t\t\t<span>{{proto_prop}}</span><br/> \t\t\t\tHelper argument: <br/> \t\t\t\t\t<span>{{print_prop own_prop}}</span><br/> \t\t\t\t\t<span>{{print_prop constructor.static_prop}}</span><br/> \t\t\t\t\t<span>{{print_prop constructor.proto_prop}}</span><br/> \t\t\t\t\t<span>{{print_prop proto_prop}}</span><br/> \t\t\t\tHelper hash argument: <br/> \t\t\t\t\t<span>{{print_hash prop=own_prop}}</span><br/> \t\t\t\t\t<span>{{print_hash prop=constructor.static_prop}}</span><br/> \t\t\t\t\t<span>{{print_hash prop=constructor.proto_prop}}</span><br/> \t\t\t\t\t<span>{{print_hash prop=proto_prop}}</span><br/>', renderer = can.view.mustache(template), data = new can.Foo({ own_prop: 'quux' }), div = document.createElement('div');
        div.appendChild(renderer(data, {
            print_prop: function () {
                return can.map(can.makeArray(arguments).slice(0, arguments.length - 1), function (arg) {
                    while (arg && arg.isComputed) {
    ...
    test('{{#each}} handles an undefined list changing to a defined list (#629)', function () {
        var renderer = can.view.mustache('    {{description}}:     <ul>     {{#each list}}         <li>{{name}}</li>     {{/each}}     </ul>');
        var div = document.createElement('div'), data1 = new can.Map({ description: 'Each without list' }), data2 = new can.Map({
                description: 'Each with empty list',
                list: []
            });
    ...
    test('can.compute should live bind when the value is changed to a Construct (#638)', function () {
        var renderer = can.view.mustache('<p>{{#counter}} Clicked <span>{{count}}</span> times {{/counter}}</p>'), div = document.createElement('div'), counter = can.compute(), data = { counter: counter };
        div.appendChild(renderer(data));
        equal(div.getElementsByTagName('span').length, 0);
        stop();
        setTimeout(function () {
    ...
    test('@index in partials loaded from script templates', function () {
        ...
        script.text = '<label></label>';
        document.body.appendChild(script);
        var itemsTemplate = can.view.mustache('<div>' + '{{#each items}}' + '{{>itempartial}}' + '{{/each}}' + '</div>');
        var items = new can.List([
                {},
                {}
            ]);
    ...
    test('Mustache helper: if w/ each removing all content', function () {
        var expected = '123content', container = new can.Map({
                items: [
                    1,
                    2,
                    3
    ...
    test('Inverse ^if should work with an else clause (#751)', function () {
        var tmpl = '{{^if show}}' + '<div>Not showing</div>' + '{{else}}' + '<div>Is showing</div>' + '{{/if}}';
    ...
    test('Expandos (#744)', function () {
        var template = can.mustache('{{#each items}}<div>{{name}}</div>{{/each}}' + '{{#if items.spliced}}<strong>List was spliced</strong>{{/if}}');
        var items = new can.List([{ name: 1 }]);
    ...
    test('Calling .fn without arguments should forward scope by default (#658)', function () {
        var tmpl = '{{#foo}}<span>{{bar}}</span>{{/foo}}';
        var frag = can.mustache(tmpl)(new can.Map({ bar: 'baz' }), {
                foo: function (opts) {
                    return opts.fn();
                }
            });
    ...
    test('can.Construct derived classes should be considered objects, not functions (#450)', 8, function () {
        ...
        var foostructor = can.Map({ text: 'bar' }, {}), obj = {
            ...
            }, div = document.createElement('div'), description;
        foostructor.self = foostructor;
        window.other_text = 'Window context';
        for (var i = 0; i < 2; i++) {
            if (i === 1) {
        ...
        equal(content[1].innerHTML.replace(/<\/?span>/gi, ''), '', 'with attribute nested' + description);
...
define('can/route/pushstate/pushstate_test', [
...
/*can/model/queue/queue_test*/
define('can/model/queue/queue_test', [
    'can/util/util',
    'can/model/model',
...
/*can/construct/super/super_test*/
define('can/construct/super/super_test', ['can/construct/super/super'], function () {
...
/*can/construct/proxy/proxy_test*/
define('can/construct/proxy/proxy_test', [
...
/*can/map/lazy/map_test*/
define('can/map/lazy/map_test', [
    'can/map/lazy/lazy',
    'can/compute/compute',
    'can/test/test'
], function (undefined) {
...
/*can/map/lazy/observe_test*/
define('can/map/lazy/observe_test', [
    'can/util/util',
    'can/observe/observe',
    'can/map/lazy/lazy',
    'can/test/test'
...
/*can/map/lazy/list_test*/
define('can/map/lazy/list_test', [
    'can/util/util',
    'can/test/test'
], function () {
...
/*can/map/lazy/lazy_test*/
define('can/map/lazy/lazy_test', [
    'can/map/lazy/nested_reference_test',
    'can/map/lazy/map_test',
    'can/map/lazy/observe_test',
    'can/map/lazy/list_test'
...
/*can/map/delegate/delegate_test*/
define('can/map/delegate/delegate_test', [
...
/*can/map/setter/setter_test*/
define('can/map/setter/setter_test', [
...
define('can/map/attributes/attributes_test', [
    ...
    test('default converters', function () {
        ...
        equal(can.Map.convert.date(num).getTime(), num, 'converted to a date with a number');
        var str = 'Dec 25, 1995';
    ...
    test('defaults', function () {
        ...
        var link = new Zelda({ rupees: 255 });
        equal(link.attr('sword'), 'Wooden Sword');
        equal(link.attr('rupees'), 255);
    });
    test('nested model attr', function () {
        can.Model('NestedAttrTest.User', {}, {});
        can.Model('NestedAttrTest.Task', { attributes: { owner: 'NestedAttrTest.User.model' } }, {});
        can.Model('NestedAttrTest.Project', { attributes: { creator: 'NestedAttrTest.User.model' } }, {});
    ...
    test('attr() should respect convert functions for lists when updating', function () {
        ...
        can.Model('ListTest.Task', { attributes: { project: 'ListTest.Project.model' } }, {});
        can.Model('ListTest.Project', { attributes: { members: 'ListTest.User.models' } }, {});
    ...
    test('plugin passes old value to converter', 2, function () {
        var Ob = can.Map('AttrOldVal', {
                ...
                attributes: { test: 'AttrOldVal.oldVal' }
            }, {});
        var o = new Ob({ test: 'first' });
        o.attr('test', 'second');
    });
    test('attr does not blow away old observable when going from empty to having items (#160)', function () {
        ...
        can.Model('EmptyListTest.Project', { attributes: { members: 'EmptyListTest.User.models' } }, {});
    ...
    test('Nested converters called twice (#174)', function () {
        ...
        Thing = can.Model({
            attributes: { otherThing: 'OtherThing.model' },
...
/*can/map/validations/validations_test*/
define('can/map/validations/validations_test', [
...
/*can/map/backup/backup_test*/
define('can/map/backup/backup_test', [
...
/*can/map/list/list_test*/
define('can/map/list/list_test', ['can/map/list/list'], function () {
...
/*can/map/define/define_test*/
define('can/map/define/define_test', [
...
/*can/map/sort/sort_test*/
define('can/map/sort/sort_test', [
    'can/map/sort/sort',
    'can/test/test',
    'can/view/mustache/mustache'
], function () {
    ...
    test('live binding with comparator (#170)', function () {
        var renderer = can.view.mustache('<ul>{{#items}}<li>{{text}}</li>{{/items}}</ul>'), el = document.createElement('div'), items = new can.List([{ text: 'First' }]);
        el.appendChild(renderer({ items: items }));
        equal(el.getElementsByTagName('li').length, 1, 'Found one li');
        items.push({ text: 'Second' });
        equal(el.getElementsByTagName('li').length, 2, 'Live binding rendered second li');
    });
});
/*can/control/plugin/plugin_test*/
define('can/control/plugin/plugin_test', ['can/control/plugin/plugin'], function () {
    if (!window.jQuery) {
        return;
    }
...
/*can/view/modifiers/modifiers_test*/
define('can/view/modifiers/modifiers_test', [
    'can/map/map',
    'can/view/ejs/ejs',
    ...
    test('modifier with a deferred', function () {
        ...
        $('#qunit-test-area').html(can.test.path('view/test/deferred.ejs'), foo);
        var templateLoaded = new can.Deferred(), id = can.view.toId(can.test.path('view/test/deferred.ejs'));
        setTimeout(function () {
            foo.resolve({ foo: 'FOO' });
        }, 1);
        var check = function () {
        ...
        can.when(foo, templateLoaded).then(function (foo) {
            setTimeout(function () {
                equal($('#qunit-test-area').html(), 'FOO', 'worked!');
                start();
            }, 10);
        });
    });
...
/*can/util/object/object_test*/
define('can/util/object/object_test', ['can/util/object/object'], function () {
...
/*can/util/fixture/fixture_test*/
define('can/util/fixture/fixture_test', [
    'can/util/fixture/fixture',
    'can/model/model',
    'can/test/test'
], function () {
...
/*can/view/bindings/bindings_test*/
define('can/view/bindings/bindings_test', [
    ...
    asyncTest('can-value select remove from DOM', function () {
        expect(1);
        var template = can.view.mustache('<select can-value=\'color\'>' + '<option value=\'red\'>Red</option>' + '<option value=\'green\'>Green</option>' + '</select>'), frag = template(), ta = document.getElementById('qunit-test-area');
        ta.appendChild(frag);
    ...
    test('checkboxes with can-value bind properly (#628)', function () {
        var data = new can.Map({ completed: true }), frag = can.view.mustache('<input type="checkbox" can-value="completed"/>')(data);
        can.append(can.$('#qunit-test-area'), frag);
    ...
    test('checkboxes with can-true-value bind properly', function () {
        var data = new can.Map({ sex: 'male' }), frag = can.view.mustache('<input type="checkbox" can-value="sex" can-true-value="male" can-false-value="female"/>')(data);
        can.append(can.$('#qunit-test-area'), frag);
        var input = can.$('#qunit-test-area')[0].getElementsByTagName('input')[0];
    ...
    test('can-value select multiple with values seperated by a ;', function () {
        var template = can.view.mustache('<select can-value=\'color\' multiple>' + '<option value=\'red\'>Red</option>' + '<option value=\'green\'>Green</option>' + '<option value=\'ultraviolet\'>Ultraviolet</option>' + '</select>');
        var map = new can.Map({ color: 'red' });
        var frag = template(map);
        var ta = document.getElementById('qunit-test-area');
        ta.appendChild(frag);
    ...
    test('can-value select multiple with values cross bound to an array', function () {
        var template = can.view.mustache('<select can-value=\'colors\' multiple>' + '<option value=\'red\'>Red</option>' + '<option value=\'green\'>Green</option>' + '<option value=\'ultraviolet\'>Ultraviolet</option>' + '</select>');
        var map = new can.Map({});
        var frag = template(map);
        var ta = document.getElementById('qunit-test-area');
        ta.appendChild(frag);
    ...
    test('can-event handlers work with {} (#905)', function () {
        expect(4);
        var template = can.mustache('<div>' + '{{#each foodTypes}}' + '<p can-click=\'{doSomething}\'>{{content}}</p>' + '{{/each}}' + '</div>');
        var foodTypes = new can.List([
                {
                    title: 'Fruits',
                    content: 'oranges, apples'
    ...
    test('can-value select with null or undefined value (#813)', function () {
        var template = can.view.mustache('<select id=\'null-select\' can-value=\'color-1\'>' + '<option value=\'\'>Choose</option>' + '<option value=\'red\'>Red</option>' + '<option value=\'green\'>Green</option>' + '</select>' + '<select id=\'undefined-select\' can-value=\'color-2\'>' + '<option value=\'\'>Choose</option>' + '<option value=\'red\'>Red</option>' + '<option value=\'green\'>Green</option>' + '</select>');
        var map = new can.Map({
                'color-1': null,
                'color-2': undefined
            });
    ...
    test('radio type conversion (#811)', function () {
        var data = new can.Map({ id: 1 }), frag = can.view.mustache('<input type="radio" can-value="id" value="1"/>')(data);
        can.append(can.$('#qunit-test-area'), frag);
        var input = can.$('#qunit-test-area')[0].getElementsByTagName('input')[0];
...
define('can/view/live/live_test', [
    'can/view/live/live',
    'can/observe/observe',
    'can/test/test'
], function () {
...
/*can/view/scope/scope_test*/
define('can/view/scope/scope_test', [
    ...
    test('use highest default observe in stack unless you\'ve found your way in something that does exist', function () {
        var bottom = new can.Map({ name: { first: 'Justin' } });
        var middle = new can.Map({ name: { first: 'Brian' } });
        var top = new can.Map({ title: 'top' });
...
/*can/util/string/string_test*/
define('can/util/string/string_test', ['can/util/string/string'], function () {
...
/*can/util/attr/attr_test*/
define('can/util/attr/attr_test', [
    'can/util/util',
    'can/util/attr/attr'
], function () {
    ...
    test('template attr updating', function () {
        var template = can.view.mustache('<div my-attr=\'{{value}}\'></div>'), compute = can.compute('foo');
