...
var behaviors = {
    ...
    value: function (el, data) {
        var propName = '$value', attrValue = str.trim(removeBrackets(el.getAttribute('can-value'))), getterSetter;
...
var expression = {
    ...
    tokenize: function (expression) {
        var tokens = [];
        (str.trim(expression) + ' ').replace(tokensRegExp, function (whole, arg) {
            tokens.push(arg);
        });
        return tokens;
    },
...
var core = {
    ...
    makeLiveBindingPartialRenderer: function (partialName, state) {
        partialName = str.trim(partialName);
    ...
    splitModeFromExpression: function (expression, state) {
        ...
        if ('#/{&^>!'.indexOf(mode) >= 0) {
            expression = str.trim(expression.substr(1));
        } else {
            mode = null;
        }
        if (mode === '{' && state.node) {
