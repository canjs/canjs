...
Create a compute that derives its value from [can.Observe]s and other [can.computed can.compute]s.
...
produces that value. This compute will automatically update its value when any [can.Observe observable]
values are read via [can.Observe.prototype.attr].
...
to [can.Observe Observes], but they represent a single value rather than 
...
[can.Observe]s, and [can.Observe.List]s. When the derived values
...
value from properties of a [can.Observe] or other [can.compute]s, the compute will listen for changes in those
...
Notice how the definition of the compute uses `[can.Observe.prototype.attr attr]`
