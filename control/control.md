...
The control's associated [can.stache stache] template looks like:

    {{#each todo}}
      <li>
        {{ name }}
        <a href="javascript://" class="destroy" ($click)="destroy">
      </li>
    {{/each}}
...
is an instance of [can.List]:
    ...
    new Todos( '#todos', { template: 'specialTodos.stache' } );
...
When the todo is destroyed, stache's live binding will remove its LI automatically.
...
This is convenient when listening for model changes. If stache were not
