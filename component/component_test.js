...
test('multiple insertion points in a list', function(){
	can.Component.extend({
		tag: "my-list",
...
test('multiple insertion points with component in another component', function(){
	can.Component.extend({
		tag: "my-list",
		template: "{{#each items}}<li><content select='.index'></content><content select='.seperator'></content><content select='.name'></content></li>{{/each}}",
		scope: {
			items: new can.List([{name: "one"},{name: "two"}])
		},
		events : {
	...
	var template = can.view.mustache("  <my-list><my-seperator></my-seperator><span class='name'>{{name}}</span><span class='index'>{{@index}}</span></my-list>  ");
	
	
	can.append(can.$("#qunit-test-area"), template({}));

	equal(stripComments(can.$('#qunit-test-area my-list li')[0].innerHTML), '<span class="index">0</span><span class="seperator">.</span><span class="name">one</span>');

	equal(stripComments(can.$('#qunit-test-area my-list li')[1].innerHTML), '<span class="index">1</span><span class="seperator">.</span><span class="name">two</span>');

	can.trigger( can.$("#qunit-test-area my-list"), "click" );

	equal(stripComments(can.$('#qunit-test-area my-list li')[0].innerHTML), '<span class="index">0</span><span class="seperator">.</span><span class="name">one</span>');

	equal(stripComments(can.$('#qunit-test-area my-list li')[2].innerHTML), '<span class="index">2</span><span class="seperator">.</span><span class="name">three</span>');
