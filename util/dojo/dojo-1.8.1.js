...
(function (userConfig, defaultConfig) {
	...
	//
	var listenerQueues = {}, error = 'error',
		...
		var consumePendingCacheInsert = function (referenceModule) {
			...
			config = function (config, booting, referenceModule) {
				for (var p in config) {
					...
					if (p === 'cacheBust') {
						cacheBust = config[p] ? isString(config[p]) ? config[p] : new Date()
							.getTime() + '' : '';
					}
	...
	var injectDependencies = function (module) {
		...
		defQ = [],
		waiting = {}, setRequested = function (module) {
		...
		runFactory = function (module, args) {
			...
			if (has('config-dojo-loader-catches')) {
				try {
					...
					signal(error, module.result = makeError('factoryThrew', [
						module,
						e
					]));
				}
		...
		execModule = function (module, strict) {
			...
			if (module.executed === executing) {
				req.trace('loader-circular-dependency', [circleTrace.concat(module.mid)
					.join('->')
				]);
			...
			evalModuleText = function (text, module) {
				...
				if (1 && legacyMode) {
					if (module.isXd) {
						...
						legacyMode === sync && (legacyMode = xd); // fall through and load via script injection
						...
						if (has('config-dojo-loader-catches')) {
							try {
								...
								signal(error, makeError('xhrInjectFailed', [
									module,
									e
								]));
							}
				...
				req.trace('loader-define-module', [
					module.mid,
					deps
				]);
			...
			req.injectUrl = function (url, callback, owner) {
				...
				var node = owner.node = doc.createElement('script'),
					...
					errorDisconnector = domOn(node, 'error', 'onerror', function (e) {
						loadDisconnector();
						errorDisconnector();
						signal(error, makeError('scriptError', [
							url,
							e
						]));
					});
	...
	cache: {
			...
			define([
				...
				var form = {
					...
					toObject: function formToObject(formNode) {
						...
						//		|		]
						//		|	};
						var ret = {}, elems = dom.byId(formNode)
								.elements;
		...
		'dojo/i18n': function () {
			define([
				...
				var thisModule = dojo.i18n = {}, nlsRe = /(^.*(^|\/)nls)(\/|$)([^\/]*)\/?([^\/]*)/,
             ...
             =====*/
				...
				if (1) {
					dojo.moduleUrl = function (module, url) {
						...
						var result = null;
						if (module) {
							result = require.toUrl(module.replace(/\./g, '/') + (url ? '/' + url : '') + '/*.*')
								.replace(/\/\*\.\*/, '') + (url ? '' : '/');
						}
						return result;
					};
		...
		'dojo/NodeList-dom': function () {
			define([
				...
				'./dom-attr',
				'./dom-style'
			], function (dojo, query, array, lang, domCls, domCtr, domGeom, domAttr, domStyle) {
				// module:
             ...
             =====*/
				...
				lang.extend(NodeList, {
					...
					orphan: function (filter) {
						...
						return (filter ? query._filterResult(this, filter) : this)
							.forEach(orphan); // dojo/NodeList
					},
					adopt: function (queryOrListOrNode, position) {
						...
						//
						//		or an offset in the childNodes property
						return query(queryOrListOrNode)
			...
			define([
				...
				var makeErrorToken = function (id) {
								...
								var callingModule = require.module,
									...
									i18nDeps, syncLoaderApi = {
										...
										requireLocalization: function (moduleName, bundleName, locale) {
											...
											locale = (locale || dojo.locale)
												.toLowerCase();
											moduleName = slashName(moduleName) + '/nls/' + (/root/i.test(locale) ? '' : locale + '/') + slashName(bundleName);
											if (getModule(moduleName, callingModule)
												.isXd) {
												// don't have to map since that will occur when the dependency is resolved
					...
					extractLegacyApiApplications = function (text, noCommentText) {
						...
						for (var p in getModule('dojo', module)
							.result.scopeMap) {
							names.push(p);
				...
				dojo.platformRequire = function (modMap) {
					...
					//		|	});
					var result = (modMap.common || [])
						.concat(modMap[dojo._name] || modMap['default'] || []),
						temp;
					while (result.length) {
				...
				if (has('ie')) {
					...
					dom.byId = function (id, doc) {
						...
						return (typeof id === 'string' ? (doc || win.doc)
							.getElementById(id) : id) || null; // DOMNode
					};
				}
				/*=====
                 dom.byId = function(id, doc){
             ...
             */
				...
				var getQueryParts = function (query) {
					...
					var endClass = function () {
						...
						if (inClass >= 0) {
							currentPart.classes.push(ts(inClass + 1, x)
								.replace(/\\/g, ''));
							inClass = -1;
						}
					};
				...
				var query = function (query, root) {
					...
					caseSensitive = od.createElement('div')
						.tagName === 'div';
					// NOTE:
		...
		'dojo/dom-geometry': function () {
			define([
				...
				function isButtonTag(node) {
					...
					return node.tagName.toLowerCase() === 'button' || node.tagName.toLowerCase() === 'input' && (node.getAttribute('type') || '')
						.toLowerCase() === 'button'; // boolean
				}
			...
			define([
				...
				for (var param in tagWrap) {
					if (tagWrap.hasOwnProperty(param)) {
						...
						tw.post = '</' + tw.reverse()
							.join('></') + '>'; // the last line is destructive: it reverses the array,
			...
			define('dojo/_base/lang', [
				...
				var _extraNames = has('bug-for-in-skips-shadowed') ? 'hasOwnProperty.valueOf.isPrototypeOf.propertyIsEnumerable.toLocaleString.toString.constructor'.split('.') : [],
					...
					efficient = function (obj, offset, startWith) {
						return (startWith || [])
							.concat(Array.prototype.slice.call(obj, offset || 0));
					}, _pattern = /\{([^\}]+)\}/g;
				// Module export
				var lang = {
					...
					_toArray: has('ie') ? function () {
						...
						return function (obj) {
							return (obj.item ? slow : efficient)
		...
		'dojo/request/util': function () {
			define('dojo/request/util', [
				...
				exports.deferred = function deferred(response, cancel, isValid, isReady, handleResponse, last) {
					...
					var responsePromise = def.then(okHandler)
				...
				exports.addCommonMethods = function addCommonMethods(provider, methods) {
					array.forEach(methods || [
						...
						provider[(method === 'DELETE' ? 'DEL' : method)
							.toLowerCase()] = function (url, options) {
				...
				exports.checkStatus = function (stat) {
					stat = stat || 0;
					return stat >= 200 && stat < 300 || stat === 304 || stat === 1223 || !stat; // or, we're Titanium/browser chrome/chrome extension requesting a local file
				};
			});
		},
			...
			define([
				...
				function addListener(target, type, listener, dontFix, matchesTarget) {
					...
					if (selector) {
						...
						return on.selector(selector, type)
				...
				if (has('dom-addeventlistener')) {
					...
					on._fixEvent = function (evt, sender) {
						...
						if (!evt) {
							var w = sender && (sender.ownerDocument || sender.document || sender)
								.parentWindow || window;
							evt = w.event;
						}
						if (!evt) {
		...
		'dojo/_base/sniff': function () {
			define([
             ...
             =====*/
				...
				dojo.locale = dojo.locale || (has('ie') ? navigator.userLanguage : navigator.language)
					.toLowerCase();
				return has;
			});
		},
             ...
             */
				...
				cls = {
					contains: function containsClass(node, classStr) {
						...
						return (' ' + dom.byId(node)[className] + ' ')
							.indexOf(' ' + classStr + ' ') >= 0; // Boolean
					},
					add: function addClass(node, classStr) {
						// summary:
					...
					if (around) {
						...
						signal = {
							...
							advice: function (target, args) {
								return signal.cancelled ? previous.advice(target, args) : advised.apply(target, args); // called the advised function
							}
						};
					} else {
				...
				has.add('events-keypress-typed', function () {
					...
					try {
						...
						(testKeyEvent.initKeyboardEvent || testKeyEvent.initKeyEvent)
							.call(testKeyEvent, 'keypress', true, true, null, false, false, false, false, 9, 3);
					} catch (e) {}
