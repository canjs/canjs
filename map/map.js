...
steal('can/util','can/util/bind','can/construct', 'can/util/batch',function(can, bind) {
	 ...
	 */
	//
	var Map = can.Map = can.Observe = can.Construct.extend( {
	/**
	 * @static
	 */
		setup: function(){
		...
		keys: function(map) {
			var keys = [];
			Map.__reading && Map.__reading(map, '__keys');
	 ...
	 */
	{
		...
		attr: function( attr, val ) {
			...
			if ( type !== "string" && type !== "number" ) {
				...
				Map.__reading && Map.__reading(this, attr)
		 ...
		 */
		each: function() {
			Map.__reading && Map.__reading(this, '__keys');
