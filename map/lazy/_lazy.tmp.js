	...
	can.LazyMap = can.Map.extend({
		...
		_addChild: function (path, newChild, setNewChild) {
				...
				if (newChild) {
					...
					if (path === newChildPath) {
						...
						oldChild._nestedReference.each(function (obj, path) {
							...
							if(self._bindings) {
								bindToChildAndBubbleToParent(obj, path, newChild);
							}
						});
						//
					} else {
						...
						if(self._bindings) {
							bindToChildAndBubbleToParent(oldChild, reference, newChild);
						}
					}
			...
			if (newChild) {
				...
				if(this._bindings) {
					bindToChildAndBubbleToParent(newChild, reference, this);
				}
			}
			return newChild;
		},
