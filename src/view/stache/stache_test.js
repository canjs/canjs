...
steal("can/view/stache", "can/view","can/test","can/view/mustache/spec/specs","steal-qunit",function(){
	...
	test("Partials and observes", function () {
		...
		template = can.stache("<table><thead><tr>{{#data}}{{>" +
			can.test.path('src/view/stache/test/partial.stache') + "}}{{/data}}</tr></thead></table>")
	...
	test("recursive views", function () {
		...
		var div = document.createElement('div');
		div.appendChild(can.view(can.test.path('src/view/stache/test/recursive.stache'), {
			items: data
		}));
	...
	test("can pass in partials", function () {
		var hello = can.view(can.test.path('src/view/stache/test/hello.stache'));
		var fancyName = can.view(can.test.path('src/view/stache/test/fancy_name.stache'));
		var result = hello({
			name: "World"
		}, {
			partials: {
