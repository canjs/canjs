	...
	if (window.history && history.pushState) {
		...
		if (typeof steal !== 'undefined') {
			...
			var makeTestingIframe = function (callback) {
				...
				iframe.src = can.test.path("src/route/pushstate/testing.html")+"?" + Math.random();
			...
			test("unsticky routes", function () {
				...
				var iframe = document.createElement('iframe');
				iframe.src = can.test.path("src/route/pushstate/testing.html?1");
				can.$("#qunit-fixture")[0].appendChild(iframe);
			});
			...
			test("clicked hashes work (#259)", function () {
				...
				var iframe = document.createElement('iframe');
				iframe.src = can.test.path("src/route/pushstate/testing.html");
				can.$("#qunit-fixture")[0].appendChild(iframe);
			});
			...
			if(window.parent === window) {
				...
				test("root can include the domain", function () {
					...
					stop();
					makeTestingIframe(function(info, done){
						info.route.bindings.pushstate.root = can.test.path("src/route/pushstate/testing.html", true).replace("src/route/pushstate/testing.html", "");
						info.route("src/:module/:plugin/:page\\.html");
						info.route.ready();
